...
# --- 5. หน้าสำหรับ User (รองรับการพิมพ์ค้นหาใน Dropdown) ---
if page == "หน้าสำหรับ User":
    ...
    with st.form("user_form", clear_on_submit=True):
        ...
        # Selectbox ของ Streamlit สามารถพิมพ์ค้นหาได้ทันที
        uni = st.selectbox("มหาวิทยาลัย / สถาบัน (พิมพ์ค้นหาได้ที่นี่)", options=["-- เลือกมหาวิทยาลัย --"] + list_uni)
        col_fac, col_maj = st.columns(2)
        faculty = col_fac.text_input("คณะ")
        major = col_maj.text_input("สาขาวิชา")
        
        affiliation = st.selectbox("สังกัด / หน่วยงาน (พิมพ์ค้นหาได้ที่นี่)", options=["-- เลือกหน่วยงาน --"] + list_agency)
        ...
        if st.form_submit_button("ส่งข้อมูล", type="primary"):
            if not (up_file and f_name and phone and uni != "-- เลือกมหาวิทยาลัย --"):
                ...
                try:
                    ...
                    # ลำดับข้อมูลให้ตรงกับหัวตารางใน GSheet (คอลัมน์ A: คำนำหน้า, B: ชื่อ, C: นามสกุล ...)
...
# --- 6. หน้าสำหรับ Admin & จัดการรายชื่อ ---
    ...
    if not st.session_state.get('logged_in', False):
        ...
        if page == "หน้าสำหรับ Admin":
            try:
                # แก้ปัญหาเรื่องชื่อคอลัมน์ 'Filename'
                ...
                if len(raw_data) > 1:
                    ...
                    # หาคอลัมน์ที่เป็น Filename (สมมติว่าเป็นคอลัมน์ที่ 12 ถ้าหาชื่อไม่เจอ)
                    ...
                    if selected_file:
                        ...
                        with c2:
                            link_val = row_info.iloc[-1] # ลิงก์ผลงานอยู่คอลัมน์สุดท้าย
        ...
        # --- ส่วนจัดการรายชื่อมหาวิทยาลัย/หน่วยงาน ---
        else:
