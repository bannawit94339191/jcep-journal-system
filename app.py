        ...
        if st.button("🚀 ส่งข้อมูลวารสาร", type="primary", use_container_width=True):
            if not (f_name and phone and email and up_file and uni):
                ...
                try:
                    ...
                    # เมื่อบันทึกสำเร็จ ล้างค่า session state ทั้งหมด
...
# --- 9. หน้าเพิ่มข้อมูลมหาวิทยาลัย / หน่วยงาน (แก้พาดหัว + ล้างค่าแน่นอน) ---
    ...
    st.markdown(f"# ⚙️ จัดการข้อมูล{target}") # แก้พาดหัวให้ตรงหน้าแล้ว
    
    if st.button("⬅️ กลับหน้าส่งวารสาร"):
        st.session_state.nav_state = menu_options[0]
        st.rerun()
    
    # ระบบล้างฟอร์มโดยใช้ Form Key
        ...
        if st.form_submit_button("🚀 บันทึกข้อมูล", type="primary"):
            if n:
                try:
                    ...
                    st.cache_data.clear()
                    st.session_state.form_counter += 1 # เปลี่ยน Key เพื่อ Reset Form
