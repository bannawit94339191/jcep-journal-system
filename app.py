...
def show_message_modal(text):
    ...
    if st.button("ปิดหน้าต่างและล้างค่า", use_container_width=True):
        st.cache_data.clear()
        # ล้างค่าใน session state เพื่อ Reset Form
...
if st.session_state.nav_state == "หน้าสำหรับกรอกข้อมูล":
    ...
    st.info("💡 กรณีไม่พบรายชื่อหน่วยงาน/มหาวิทยาลัย โปรดเลือกเมนูด้านล่างเพื่อเพิ่มข้อมูล")
    ...
    with st.container(border=True):
        ...
        phone = col_t.text_input("เบอร์โทรศัพท์ (ไม่ต้องมีขีด)", value=auto_phone)
        ...
        if st.button("🚀 ส่งข้อมูลวารสาร", type="primary", use_container_width=True):
            if not (f_name and phone and email and up_file):
                ...
                try:
                    ...
                    # ส่งข้อมูลไป Sheets (จับคู่ให้ตรงคอลัมน์)
                    # ลำดับ: ID, คำนำหน้า, ชื่อ, นามสกุล, มหาลัย, สังกัด, ที่อยู่, เบอร์, อีเมล, ประเภท, ชื่อไฟล์, ลิงก์, วันที่
                    sheet.append_row([
                    ...
                    show_message_modal(f"✅ ส่งข้อมูลสำเร็จ!<br>เลขที่รับเรื่อง: <b>{unique_id}</b>")
                except Exception as e: st.error(f"Error: {e}")

    # --- ปุ่มทางลัด 2 ปุ่มด้านล่าง (ตามรูปภาพ) ---
    st.markdown("<br>", unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    if col1.button("🏠 ไปหน้า: เพิ่มมหาวิทยาลัย", type="secondary", use_container_width=True):
        st.session_state.nav_state = "เพิ่มข้อมูลมหาวิทยาลัย/สถาบัน"
        st.rerun()
    if col2.button("🏢 ไปหน้า: เพิ่มหน่วยงาน", type="secondary", use_container_width=True):
...
# --- 10. หน้าจัดการรายชื่อ (สถาบัน/หน่วยงาน) ---
    ...
    with st.form("add_list_form", clear_on_submit=True):
        ...
        if st.form_submit_button("🚀 บันทึกรายชื่อใหม่", type="primary"):
            if n:
                try:
                    info = st.secrets["google_auth"]
                    creds = service_account.Credentials.from_service_account_info(info)
