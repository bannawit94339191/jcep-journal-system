...
if st.session_state.nav_state == "หน้าสำหรับกรอกข้อมูล":
    ...
    with st.container(border=True):
        ...
        f_name = col_f.text_input("ชื่อ", key="input_fname")
        l_name = col_l.text_input("นามสกุล", key="input_lname")
        
        uni = st.selectbox("มหาวิทยาลัย / สถาบัน", options=list_uni, index=None, placeholder="เลือกมหาวิทยาลัย...")
        
        col_fac, col_maj = st.columns(2)
        faculty = col_fac.text_input("คณะ", key="input_faculty")
        major = col_maj.text_input("สาขาวิชา", key="input_major")
        ...
        if uni and not df_uni.empty:
            ...
            if not row.empty: 
                # ดึงข้อมูลจากคอลัมน์ Index 1, 2, 3 (ที่อยู่, เบอร์, เมล)
        ...
        if st.button("🚀 ส่งข้อมูลวารสาร", type="primary", use_container_width=True):
            if not (f_name and l_name and phone and email and up_file and uni):
                st.warning("⚠️ กรุณากรอกข้อมูลสำคัญให้ครบถ้วน (รวมถึงนามสกุล)")
                ...
                try:
                    ...
                    # บันทึกลำดับให้ตรงกับ Google Sheet (A ถึง N)
                    # ลำดับ: ID, คำนำหน้า, ชื่อ, นามสกุล, มหาลัย, คณะ, สาขา, สังกัด, ที่อยู่, เบอร์, เมล, ประเภทบทความ, ชื่อไฟล์, วันเวลา
                    sheet.append_row([
                    ...
                    for key in list(st.session_state.keys()):
                        if key not in ['nav_state', 'logged_in']: del st.session_state[key]
...
# --- 8. หน้า Admin ---
    ...
    if not st.session_state.get('logged_in', False):
        ...
        except Exception as e: st.error(f"ดึงข้อมูลไม่สำเร็จ: {e}")

# --- 9. หน้าเพิ่มข้อมูลสถาบัน ---
