...
# --- 5. ข้อมูล Admin (Username & Password) ---
ADMIN_USERS = {
    "bannawit.s": "adminjcep",
    "suphanut.p": "admin1359",
    "oris.m": "admin2081",
    "ataya.s": "admin3305",
    "wasana.d": "admin4210"
}

# --- 6. ฟังก์ชันแจ้งเตือน (Popup) ---
...
# --- 7. Sidebar (แสดงสถานะผู้ใช้) ---
with st.sidebar:
    if st.session_state.logged_in:
        st.markdown(f"""
            <div class="user-status">
                🟢 <b>สถานะ:</b> ออนไลน์<br>
                👤 <b>ผู้ใช้งาน:</b> {st.session_state.current_user}
            </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown("""
            <div class="user-status" style="border-left: 5px solid #94a3b8; background-color: #f1f5f9;">
                ⚪ <b>สถานะ:</b> บุคคลทั่วไป
            </div>
        """, unsafe_allow_html=True)
...
# --- 8. หน้าสำหรับกรอกข้อมูลส่งวารสาร ---
if st.session_state.nav_state == "หน้าสำหรับกรอกข้อมูล":
    ...
    with st.container(border=True):
        ...
        # ฟังก์ชัน Auto Fill สำหรับ มหาวิทยาลัย
        def update_uni_fill():
        ...
        # ฟังก์ชัน Auto Fill สำหรับ หน่วยงาน/สังกัด (เพิ่มใหม่ตามคำขอ)
        def update_agency_fill():
            if st.session_state.agency and not df_agency.empty:
                row = df_agency[df_agency.iloc[:, 0] == st.session_state.agency]
                if not row.empty:
                    st.session_state.addr = row.iloc[0, 1]
                    st.session_state.phone = str(row.iloc[0, 2])
                    st.session_state.email = row.iloc[0, 3]

        uni = st.selectbox("มหาวิทยาลัย / สถาบัน", options=list_uni, index=None, placeholder="เลือกมหาวิทยาลัย...", key="uni", on_change=update_uni_fill)
        
        col_fac, col_maj = st.columns(2)
        faculty = col_fac.text_input("คณะ", key="faculty")
        major = col_maj.text_input("สาขาวิชา", key="major")
        
        # เพิ่ม on_change เพื่อให้หน่วยงาน auto fill ได้เช่นกัน
        affiliation = st.selectbox("สังกัด / หน่วยงาน", options=list_agency, index=None, placeholder="เลือกหน่วยงาน...", key="agency", on_change=update_agency_fill)
        ...
        if st.button("🚀 ส่งข้อมูลวารสาร", type="primary", use_container_width=True):
            if not (f_name and l_name and phone and email and up_file and (uni or affiliation)):
                st.warning("⚠️ กรุณากรอกข้อมูลสำคัญและเลือกสถาบันหรือหน่วยงานให้ครบถ้วน")
                ...
                try:
                    ...
                    sheet.append_row([unique_id, prefix, f_name, l_name, uni if uni else "", faculty, major, affiliation if affiliation else "", address, phone, email, final_article_type, up_file.name, datetime.now().strftime("%Y-%m-%d %H:%M")])
                    show_message_modal(f"✅ ส่งข้อมูลสำเร็จ!<br>เลขรับเรื่อง: <b>{unique_id}</b>")
                except Exception as e: st.error(f"Error: {e}")

# --- 9. หน้า Admin (Dashboard) ---
    ...
    if not st.session_state.logged_in:
        ...
        if st.button("Sign In"):
            if u_input in ADMIN_USERS and p_input == ADMIN_USERS[u_input]:
        ...
        col_t.markdown("## 🖥️ Dashboard for Admin") 
...
# --- 10. หน้าจัดการข้อมูลสถาบัน/หน่วยงาน ---
