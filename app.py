...
# ฟังก์ชันล้างค่าใน Session State
def reset_form_data():
    keys_to_reset = ["fname", "lname", "uni", "faculty", "major", "agency", "addr", "phone", "email", "other_detail", "article_link"]
...
def show_message_modal(text):
    ...
    if st.button("ตกลง", use_container_width=True):
        reset_form_data() # ล้างค่าข้อมูลทั้งหมดใน State
        st.rerun()
...
if st.session_state.nav_state == "หน้าสำหรับกรอกข้อมูล":
    ...
    with st.container(border=True):
        ...
        # ส่วนสำคัญ: เมื่อเปลี่ยนชื่อมหาวิทยาลัย ให้ไปดึงข้อมูลมาเติมใน State โดยตรง
        def update_auto_fill():
            selected_uni = st.session_state.uni
            if selected_uni and not df_uni.empty:
                row = df_uni[df_uni.iloc[:, 0] == selected_uni]
                if not row.empty:
                    st.session_state.addr = row.iloc[0, 1] if len(row.columns) > 1 else ""
                    st.session_state.phone = str(row.iloc[0, 2]) if len(row.columns) > 2 else ""
                    st.session_state.email = row.iloc[0, 3] if len(row.columns) > 3 else ""

        uni = st.selectbox("มหาวิทยาลัย / สถาบัน", options=list_uni, index=None, 
                           placeholder="เลือกมหาวิทยาลัย...", key="uni", on_change=update_auto_fill)
        ...
        if st.button("🚀 ส่งข้อมูลวารสาร", type="primary", use_container_width=True):
            if not (f_name and l_name and phone and email and up_file and uni):
                ...
                try:
                    ...
                    # บันทึกข้อมูล (ตรวจสอบลำดับจากหัวตาราง)
                    sheet.append_row([
    ...
    if not st.session_state.get('logged_in', False):
        ...
        try:
            ...
            if len(raw_data) > 1:
                ...
                fig = px.pie(counts, values='จำนวน', names='ประเภท', 
                             color='ประเภท',
