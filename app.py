...
# --- 2. ระบบ Cache ข้อมูล (ช่วยแก้ปัญหา Quota Exceeded) ---
@st.cache_data(ttl=600)  # เก็บข้อมูลไว้ในหน่วยความจำ 10 นาที
def get_google_data():
    if "google_auth" in st.secrets:
        try:
            ...
            st.error(f"การเชื่อมต่อผิดพลาด (API Limit อาจเต็ม): {e}")
            return pd.DataFrame(), pd.DataFrame()
    return pd.DataFrame(), pd.DataFrame()

# เรียกใช้งานฟังก์ชัน Cache
df_uni, df_agency = get_google_data()

# เตรียมรายชื่อ (ป้องกัน NameError โดยให้เป็นค่าว่างหากโหลดไม่ได้)
...
def show_message_modal(text):
    ...
    if st.button("ปิดหน้าต่าง", use_container_width=True):
        st.cache_data.clear() # เคลียร์ cache เมื่อมีการอัปเดตข้อมูลใหม่
        st.rerun()
...
# --- 6. หน้าสำหรับ User (พร้อม Auto-fill) ---
    ...
    with st.container(border=True):
        ...
        if st.button("✅ ส่งข้อมูลวารสาร", type="primary", use_container_width=True):
            if not (up_file and f_name and phone):
                ...
                try:
                    # เชื่อมต่อเพื่อเขียนข้อมูล (ไม่ใช้ cache)
                    info = st.secrets["google_auth"]
                    creds = service_account.Credentials.from_service_account_info(info)
                    client = gspread.authorize(creds.with_scopes(['https://www.googleapis.com/auth/spreadsheets', 'https://www.googleapis.com/auth/drive']))
                ...
                except Exception as e: st.error(f"เกิดข้อผิดพลาดในการส่งข้อมูล: {e}")

# ส่วน Admin และ จัดการรายชื่อยังคงเดิม (เพิ่มการ clear cache เมื่อบันทึกสำเร็จ)
elif st.session_state.nav_state in ["จัดการรายชื่อมหาวิทยาลัย", "จัดการรายชื่อหน่วยงาน"]:
    target_name = "University" if st.session_state.nav_state == "จัดการรายชื่อมหาวิทยาลัย" else "Agency"
    label = "มหาวิทยาลัย" if target_name == "University" else "หน่วยงาน"
        ...
        if st.form_submit_button("🚀 บันทึก", type="primary"):
            if new_name:
                try:
                    ...
                    show_message_modal(f"✅ บันทึกข้อมูลสำเร็จ")
