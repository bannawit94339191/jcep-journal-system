        ...
        if st.form_submit_button("ส่งข้อมูล", type="primary"):
            if not (up_file and f_name and phone and uni != "-- เลือกมหาวิทยาลัย --" and affiliation != "-- เลือกหน่วยงาน --"):
                ...
                try:
                    ...
                    # ปรับชื่อคอลัมน์ลำดับสุดท้ายให้เป็น 'Filename' เพื่อให้ Admin หาเจอ
...
# --- 6. หน้าสำหรับ Admin & การจัดการรายชื่อ ---
    ...
    if not st.session_state.get('logged_in', False):
        ...
        if page == "หน้าสำหรับ Admin":
            try:
                # แก้ไขปัญหาไม่พบคอลัมน์ Filename
                ...
                if len(raw_data) > 1:
                    ...
                    # ตรวจสอบชื่อคอลัมน์ไฟล์ (ลำดับที่ 13 หรือชื่อ 'Filename')
                    file_col = "Filename" if "Filename" in df.columns else df.columns[12]
                    
                    st.markdown("### 📁 จัดการไฟล์และลิงก์")
                    file_list = df[file_col].dropna().unique().tolist()
                    selected_file = st.selectbox("เลือกรายการที่ต้องการดูรายละเอียด:", options=file_list, index=None)
                    
                    if selected_file:
                        row_info = df[df[file_col] == selected_file].iloc[0]
                        c1, c2 = st.columns(2)
                        with c1:
        ...
        # --- ส่วนจัดการรายชื่อ (แก้ Error Session State) ---
        else:
            ...
            # ใช้ st.form แทนการสั่งแก้ session state โดยตรงเพื่อป้องกัน error
            ...
            with st.form(f"add_form_{page}", clear_on_submit=True):
                ...
                if st.form_submit_button(f"🚀 บันทึกข้อมูล{label}", type="primary"):
                    if new_name:
                        ...
                        # แจ้งเตือนแบบ Popup (st.rerun ในฟังก์ชันจะช่วยรีเฟรชค่าเอง)
                        show_message_modal("✅ เพิ่มข้อมูลเรียบร้อย")
                    else:
                        st.warning(f"⚠️ กรุณากรอกชื่อ{label}")
            
            st.write("---")
            st.write(f"📂 รายชื่อและรายละเอียดปัจจุบัน:")
            all_rows = target_sheet.get_all_values()
            if len(all_rows) > 1:
                st.table(pd.DataFrame(all_rows[1:], columns=all_rows[0]))
            else:
