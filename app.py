...
with st.sidebar:
    st.markdown("## 🏠 HOME")
    menu_options = ["หน้าสำหรับกรอกข้อมูล", "หน้าสำหรับ Admin", "เพิ่มรายชื่อมหาวิทยาลัย/สถาบัน", "เพิ่มรายชื่อหน่วยงาน/สังกัด"]
        ...
        if st.form_submit_button("ส่งข้อมูลวารสาร", type="primary"):
            if not (up_file and f_name and phone):
                ...
                try:
                    ...
                    # เรียงลำดับข้อมูลให้ตรงตาม Google Sheets: ลำดับที่, คำนำหน้าชื่อ, ชื่อ, นามสกุล, มหาวิทยาลัย, คณะ, สาขา, สังกัด, ที่อยู่, เบอร์, เมล, ประเภท, ชื่อไฟล์
                    all_rows = sheet.get_all_values()
                    next_id = len(all_rows) # ใช้จำนวนแถวเป็นลำดับที่
                    
                    new_row = [next_id, prefix, f_name, l_name, uni, faculty, major, affiliation, address, phone, email, article_type, up_file.name]
                    sheet.append_row(new_row)
                    show_message_modal("✅ บันทึกข้อมูลของท่านเรียบร้อย")
                except Exception as e: st.error(f"เกิดข้อผิดพลาด: {e}")

# --- 6. หน้าจัดการรายชื่อ (ปลดล็อก Login) ---
...
# --- 7. หน้าสำหรับ Admin (เรียงคอลัมน์ใหม่ตาม GSheet) ---
    ...
    if not st.session_state.get('logged_in', False):
        ...
        try:
            ...
            if len(raw_data) > 1:
                # สร้าง DataFrame โดยใช้หัวจาก Google Sheets โดยตรงเพื่อความแม่นยำ
