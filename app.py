...
# --- 2. ระบบดึงข้อมูลและ Cache (แก้ปัญหา Quota และป้องกัน NameError) ---
...
def show_message_modal(text):
    ...
    if st.button("ปิดหน้าต่าง", use_container_width=True):
        st.cache_data.clear() # เคลียร์ cache เพื่อให้เห็นข้อมูลใหม่
        st.rerun()
...
# --- 6. หน้าสำหรับ User (ฟังก์ชันครบ + Auto-fill) ---
...
# --- 7. หน้าจัดการรายชื่อ (ดึงตารางกลับมาแล้ว) ---
    ...
    if not current_df.empty:
        ...
        st.warning(f"ยังไม่มีข้อมูลรายชื่อ{label}")

# --- 8. หน้าสำหรับ Admin (ดึงหัวเรื่องกลับมา + ตารางโชว์เสมอ) ---
    ...
    if not st.session_state.get('logged_in', False):
        ...
        # ส่วนหัวหน้า Admin ที่หายไป ดึงกลับมาแล้วครับ
        ...
        try:
            ...
            if len(raw_data) > 1:
                ...
                st.dataframe(pd.DataFrame(columns=headers), use_container_width=True) # โชว์หัวตารางเปล่าตามที่ต้องการ
